...
//     // float displacementX = (fbm(uv * 10.0 + time) - 0.5) * 0.3; // Adjust the strength as needed
//     // float displacementY = (fbm(uv * 10.0 + time) - 0.5) * 0.3 ; // Adjust the strength as needed
//     // Calculate the displacement amount based on time and UV coordinates
//     float scale = 8.0; // Adjust the scale of the noise
//     float speed = 0.1; // Adjust the speed of the spreading effect
//     float displacementX = noise(uv * scale + time * speed) * 0.2; // Adjust the strength of the displacement
//     float displacementY = noise(uv * scale + time * speed) * 0.2; // Adjust the strength of the displacement
...
// `;
const fragmentShader = `
// Custom fragment shader for color powder/fog effect
uniform sampler2D tDiffuse; // Input texture
uniform vec2 resolution;  // Resolution of the screen
uniform float time;     // Time for animation
uniform vec2 mouse;     // Mouse position in screen coordinates
...
float perlin(vec2 st) {
  ...
  // Four corners in 2D of a tile
  float a = smoothstep(0., 1., dot(f, f));
  float b = smoothstep(0., 1., dot(f - vec2(1.0, 0.0), f - vec2(1.0, 0.0)));
  float c = smoothstep(0., 1., dot(f - vec2(0.0, 1.0), f - vec2(0.0, 1.0)));
  float d = smoothstep(0., 1., dot(f - vec2(1.0, 1.0), f - vec2(1.0, 1.0)));

  // Mix smoothly between corners
  vec2 u = f * f * (3.0 - 2.0 * f);
...
void main() {
  ...
  // Calculate distance from center and perlin noise
  vec2 center = vec2(0.5, 0.5); // Adjust the center as needed
  float distance = length(uv - center);
  float noiseValue = perlin(uv * 8.0 + time * 0.1); // Adjust scale and speed as needed

  // Calculate displacement based on distance and noise
  float displacement = distance * noiseValue * 0.1; // Adjust strength as needed

  // Apply displacement in both X and Y directions
  uv.x += displacement;
  uv.y += displacement;

  // Sample the input texture with distorted UV coordinates
  vec4 texel = texture2D(tDiffuse, uv);

  // Define fog color (adjust as desired)
  vec3 fogColor = vec3(0.8, 0.5, 0.2); // Example: orange fog

  // Blend based on displacement (adjust factor)
  float fogFactor = smoothstep(0.0, 1.0, displacement);
  vec4 finalColor = mix(texel, vec4(fogColor, fogFactor), fogFactor);
